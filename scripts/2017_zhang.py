...
COLORIZATION_DIR = os.path.join(BASE_DIR, "models/2017_zhang")
VENV_PATH = os.path.join(COLORIZATION_DIR, "2017_zhang")
...
OUTPUT_DIR = os.path.join(BASE_DIR, "data/colorized/2017_zhang")
    ...
    for line in lines:
        ...
        if stripped_line.startswith('plt'):
            if stripped_line.startswith('plt.imsave'):
                ...
                new_line = "plt.imsave(opt.img_path, out_img_siggraph17)\n"
    ...
    for i in range(len(grayscale_images)):
        ...
        try:
            subprocess.run(
                ...
                cwd=os.path.join(BASE_DIR, "models/2017_zhang"),
                check=True
            )
